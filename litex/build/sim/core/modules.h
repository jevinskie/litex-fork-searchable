...
static inline bool clk_pos_edge(clk_edge_state_t *edge_state, int new_clk) {
  bool is_edge = edge_state->last_clk == 0 && new_clk == 1;
  edge_state->last_clk = new_clk;
  return is_edge;
}

static inline bool clk_neg_edge(clk_edge_state_t *edge_state, int new_clk) {
  bool is_edge = edge_state->last_clk == 1 && new_clk == 0;
  edge_state->last_clk = new_clk;
  return is_edge;
}

static inline clk_edge_t clk_edge(clk_edge_state_t *edge_state, int new_clk) {
