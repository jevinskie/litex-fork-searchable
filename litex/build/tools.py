...
def generated_banner(line_comment="//", reproducible=False):
    r = line_comment + "-"*80 + "\n"
    r += line_comment + " Auto-generated by Migen ({}) & LiteX ({}) on ".format(
        get_migen_git_revision() if not reproducible else "<omitted for RPB>",
        get_litex_git_revision() if not reproducible else "<omitted for RPB>")
    if not reproducible:
