...
# INI settings (quartus.ini) -----------------------------------------------------------------------

def _build_ini(additional_ini_settings):
    ini = "# Autogenerated by LiteX / git: " + tools.get_litex_git_revision() + "\n"
    ini += "\n".join(additional_ini_settings) + "\n"
    tools.write_to_file("quartus.ini", ini, force_unix=True)

# Script -------------------------------------------------------------------------------------------

def _build_script(build_name, create_rbf, dump_atoms):
...
quartus_map --read_settings_files=on  --write_settings_files=off {build_name} -c {build_name}"""
    if dump_atoms:
        script_contents += """
quartus_cdb --read_settings_files=off --write_settings_files=off {build_name} -c {build_name} --vqm={build_name}_atoms.vqm"""
    script_contents += """
...
class AlteraQuartusToolchain:
    ...
    def build(self, platform, fragment,
        ...
        **kwargs):
        ...
        # Generate quartus.ini settings
        _build_ini(self.additional_ini_settings)

        # Generate build script
        script = _build_script(build_name, platform.create_rbf, platform.dump_atoms)
